pai = [(20, '♠8'), (20, '♠9'), (28, '♣10'), (48, '♣J'), (40, '♣Q'), (18, '♣K'),
       (28, '♣A')]
...
def shunzi(pai):
    ...
    # 判断是否皇家同花顺
    if max(shuzi) - min(shuzi) == 4 and len(set(shuzi)) == 5 and len(
            set(huase)) == 1 and max(shuzi) == 12:
        jibie = 900
        return jibie

    # 判断是否同花顺
    elif max(shuzi) - min(shuzi) == 4 and len(set(shuzi)) == 5 and len(
            set(huase)) == 1 and max(shuzi) != 13:
        jibie = 800 + max(shuzi)
        return jibie
    ...
    # 判断是否同花
        ...
        jibie = 500 + lin[0] + lin[1] * 0.01 + lin[2] * 0.0001 + lin[
            3] * 0.000001 + lin[4] * 0.00000001
        return jibie

    # 判断是否顺子
    elif max(shuzi) - min(shuzi) == 4 and len(
            set(shuzi)) == 5 and max(shuzi) != 13:
        jibie = 400 + max(shuzi)
        return jibie
    ...
    # 判断是否两对
        ...
        for i in shuzi:
            if i != max(kind(2, pai)) and i != min(kind(2, pai)):
                ti = i
        jibie = 200 + max(kind(2,
                               pai)) + min(kind(2, pai)) * 0.01 + ti * 0.0001
        return jibie
        ...
        for i in shuzi:
            if i != max(kind(2, pai)):
                ti.append(i)
        jibie = 100 + max(kind(
            2, pai)) + ti[0] * 0.01 + ti[1] * 0.0001 + ti[2] * 0.000001
        return jibie
    ...
    # 判断是否普通牌
        ...
        lin = list(shuzi)
        lin.sort(reverse=True)
        jibie = 0 + lin[0] + lin[1] * 0.01 + lin[2] * 0.0001 + lin[
            3] * 0.000001 + lin[4] * 0.00000001
        return jibie
    # print(jibie)
